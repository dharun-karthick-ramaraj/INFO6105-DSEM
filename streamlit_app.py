...
@st.cache(allow_output_mutation=True)  # Adjusted for correct caching in Streamlit
def load_model():
...
def save_audio(samples: torch.Tensor):
    ...
    if samples.dim() == 2:
        samples = samples.unsqueeze(0)  # Ensure batch dimension is present
    for idx, audio in enumerate(samples):
        audio_path = os.path.join(save_path, f"audio_{idx}.wav")
        torchaudio.save(audio_path, audio, sample_rate)
    return audio_path  # Return the path for linking in Streamlit
